...
public class UpdateRecurringEventJob(ILogger<IJob<RecurringEventFoundEvent>> logger, IRecurringJobManagerV2 manager, IMediator mediator) : BaseJob<RecurringEventFoundEvent>(logger)
{
    protected override bool CheckSupport(RecurringEventFoundEvent @event)
    {
        return @event.Summary is { IsInCode: true, IsInHangfire: true, HasChanges: true };
    }
